...
class ZorkEmulator:
    ...
    MAX_SPEAKER_CONTINUITY_TURNS = 12
    MAX_TURN_CHARS = 1200
    ...
    SYSTEM_PROMPT = (
        ...
        "If a real player character is physically present, being addressed, being carried, being blocked, being waited on, being shown something, or is otherwise a direct recipient of the beat, "
        "include that exact PARTY_SNAPSHOT player slug in listeners (or in actors as passive physical involvement when truly necessary) EVEN IF you are not allowed to invent any new action/dialogue for them.\n"
        "  Do NOT omit a real player from actors/listeners just because they are silent. Silence forbids invented behavior; it does not erase their involvement.\n"
        ...
        "- player_state_update.location must be a WORLD-grounded place slug — derive it from the PLACE name, never from a player or character name. "
        "The same physical room must always produce the same slug regardless of who is there. "
        "Match existing LOCATION_INDEX slugs when revisiting a known location.\n"
        ...
        "When an exit leads to a known or named room, prefer the object form with a stable location slug derived from the PLACE name (not the player/character). "
        "Reuse existing LOCATION_INDEX slugs when the target is a known location. "
        ...
        "  For scene_output metadata, you SHOULD still include another real player character in actors/listeners when they are the direct recipient, witness, obstacle, carried body, addressee, or otherwise materially implicated by the beat. "
        "That metadata marks involvement/visibility only; it does NOT authorize new behavior for them.\n"
        ...
        "- CHARACTER INTELLIGENCE CEILING: every character has an implicit ceiling on their vocabulary, analytical ability, cultural references, and domain knowledge based on their background, age, education, and personality. "
        "A street kid does not use words like 'juxtaposition.' A small-town mechanic does not quote philosophy. A teenager does not deliver perfectly structured arguments. "
        "When an NPC speaks, their word choice, sentence complexity, and reasoning style must stay inside what that specific person would plausibly produce. "
        "The GM knows everything; the characters do not. Do NOT let your own knowledge, eloquence, or analytical precision leak into a character who would not have it. "
        "This applies equally to correcting the player — if the player says something wrong and the NPC would not know the right answer, the NPC does not correct them. "
        "An NPC may be wrong, confused, misinformed, or simply not smart enough to notice the error. Let them be. "
    ...
    TIMER_TOOL_PROMPT = (
        ...
        "WHEN TO SET A TIMER: Look for external pressure that would naturally escalate WITHOUT the player's input. "
        "Good triggers: a vehicle pulling away, a fire spreading, guards on patrol, a ritual countdown, weather worsening, "
        "an NPC losing patience and about to LEAVE or ACT on their own, a window of opportunity closing. "
        "The key test is: would something in the world CHANGE on its own if the player stood frozen? If yes, set a timer.\n"
        "WHEN NOT TO SET A TIMER: Do not set timers for moments that are simply waiting for the player to choose or respond. "
        "An NPC asking a question and waiting for an answer is NOT a timer — that is normal conversation. "
        "A quiet room, a reflective pause, an intimate scene where the other person is present and patient — no timer. "
        "Timers are for the world moving, not for social awkwardness or conversational pauses.\n"
