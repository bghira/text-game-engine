...
def test_ready_to_write_finalization_uses_final_stage_system_prompt(
    ...
    async def run_test():
        ...
        assert "CHARACTER EVOLUTION CHECKPOINT" in completion.calls[1]["system_prompt"]
        assert "character_updates is REQUIRED for that NPC" in completion.calls[1]["system_prompt"]
        assert "Do not rely on narration, reasoning, or summary_update alone for durable character growth" in completion.calls[1]["system_prompt"]
        assert "Autobiographies are starting conditions, not reset buttons" in completion.calls[1]["system_prompt"]
        assert "ACTIVE_PLOT_THREADS CHECKPOINT" in completion.calls[1]["system_prompt"]
...
def test_sms_reply_nudge_clears_when_reply_arrives_on_canonical_alias_thread(
    session_factory,
    seed_campaign_and_actor,
):
    compat = _build_compat(session_factory)
...
def test_sms_write_normalizes_relative_player_target_to_owner_thread(
    session_factory,
    seed_campaign_and_actor,
):
    compat = _build_compat(session_factory)
    ...
    ok, status = compat.write_sms_thread(
        ...
        sender="Caitlin",
        recipient="Player",
        message="yeah, I saw it",
        owner_actor_id=seed_campaign_and_actor["actor_id"],
    )
    assert ok is True and status == "stored"

    canonical, _label, messages = compat.read_sms_thread(
        campaign.id,
        "caitlin",
        limit=20,
        viewer_actor_id=seed_campaign_and_actor["actor_id"],
    )
    assert canonical == "caitlin"
    assert len(messages) == 2
    assert any(
        row.get("from") == "Caitlin"
        and row.get("to") == "Chris"
        and row.get("message") == "yeah, I saw it"
        for row in messages
    )
...
def test_sms_reply_nudge_treats_legacy_relative_player_target_as_owner_alias(
    session_factory,
    seed_campaign_and_actor,
):
    compat = _build_compat(session_factory)
    ...
    with session_factory() as session:
        ...
        compat._sms_write(
            ...
            recipient="Player",
            message="yeah, I saw it",
            game_time={"day": 1, "hour": 9, "minute": 1},
            turn_id=2,
            owner_actor_id=seed_campaign_and_actor["actor_id"],
        )
    ...
    caitlin_rows = [row for row in listed if row.get("thread") == "caitlin"]
    assert len(caitlin_rows) == 1
    assert caitlin_rows[0]["count"] == 2
...
def test_sms_activity_nudge_hidden_for_player_to_player_threads(
    session_factory,
    seed_campaign_and_actor,
):
    compat = _build_compat(session_factory)
    ...
    with session_factory() as session:
        ...
        state_one = json.loads(player_one.state_json or "{}")
        state_one["character_name"] = "Sultan"
        player_one.state_json = compat._dump_json(state_one)
    ...
    with session_factory() as session:
        ...
        assert player_one is not None
        player_state = compat.get_player_state(player_one)
        row = session.get(Campaign, campaign.id)
        campaign_state = compat.get_campaign_state(row)
...
def test_recent_turns_strips_reasoning_when_reasoning_history_disabled(
    ...
    with session_factory() as session:
        ...
        session.add(
            Turn(
                campaign_id=campaign.id,
                session_id=None,
                actor_id=other_player.actor_id,
                kind="narrator",
                ...
                meta_json=json.dumps(
                    {
                        ...
                        "scene_output": {
                            ...
                            "beats": [
                                {
                                    "reasoning": "Shared local beat in the old room.",
                                    "type": "npc_dialogue",
                                    "speaker": "penny-reynolds",
                                    "actors": ["penny-reynolds"],
                                    "listeners": ["tony"],
                                    "visibility": "local",
                                    "visible_actor_ids": [player.actor_id],
                                    "aware_npc_slugs": [],
    ...
    assert "Penny says she is staying." in user_prompt
    recent_block = user_prompt.split("RECENT_TURNS:\n", 1)[1].split("\nPLAYER_ACTION ", 1)[0]
    assert '"reasoning":' not in recent_block
    ...
    with session_factory() as session:
        ...
        session.add(
            Turn(
                ...
                actor_id=player.actor_id,
                kind="player",
                content="I text Penny: call me when you can.",
                meta_json=json.dumps(
                    {
                        "game_time": {"day": 1, "hour": 9, "minute": 5},
                        "visibility": {
                            "scope": "private",
                            "actor_player_slug": "rigby",
                        },
        ...
        def fake_recent_turns_text_for_viewer(
            ...
            *,
            ...
            if focus_on_requested_receivers:
                return (
                    '{"kind":"beat","reasoning":"speaker-private context","type":"npc_dialogue",'
                    '"speaker":"simone-ashworth","text":"Private Simone continuity."}'
                )
            return (
                '{"kind":"beat","reasoning":"shared visible context","type":"npc_dialogue",'
                '"speaker":"narrator","text":"Shared room beat."}'
            )
...
def test_begin_turn_honors_channel_campaign_binding_over_stale_active_metadata(
    session_factory,
    seed_campaign_and_actor,
):
    async def run_test():
        compat = _build_compat(session_factory)
        actor_id = seed_campaign_and_actor["actor_id"]
        stale_campaign = compat.get_or_create_campaign("default", "stale", actor_id)
        bound_campaign = compat.get_or_create_campaign("default", "bound", actor_id)
        channel = compat.get_or_create_channel("default", "main")
        with session_factory() as session:
            channel_row = session.get(GameSession, channel.id)
            assert channel_row is not None
            channel_row.enabled = True
            channel_row.campaign_id = bound_campaign.id
            channel_row.metadata_json = compat._dump_json({"active_campaign_id": stale_campaign.id})
            session.commit()
        ...
        assert error is None
        assert campaign_id == bound_campaign.id
        compat.end_turn(bound_campaign.id, actor_id)

        with session_factory() as session:
            channel_row = session.get(GameSession, channel.id)
            assert channel_row is not None
            meta = compat._load_json(channel_row.metadata_json, {})
            assert meta.get("active_campaign_id") == bound_campaign.id
